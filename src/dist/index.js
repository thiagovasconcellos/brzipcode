 ...
 */
function ZipCodesFromGivenState(state) {
    switch (state) {
        case 'SP':
 ...
 * @param state  Use the abbreviation. For example "SP" for São Paulo, "RJ" for Rio de Janeiro.
 */
function isZipCodeInState(zipCode, state) {
    var realState = StateFromZipCode(zipCode);
    if (realState === state) {
        return true;
    }
    return false;
}
module.exports = { ZipCodesFromGivenState: ZipCodesFromGivenState, StateFromZipCode: StateFromZipCode, isZipCodeInState: isZipCodeInState };
